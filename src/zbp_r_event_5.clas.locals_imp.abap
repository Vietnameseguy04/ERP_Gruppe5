...
CLASS lhc_registrations IMPLEMENTATION.
  ...
  METHOD GenerateRegistrationId.
   SELECT FROM zregistration_4 FIELDS MAX( registration_id ) INTO @DATA(max_event_id).
    ...
    MODIFY ENTITY IN LOCAL MODE ZR_Registration_5
  ...
  METHOD MaxParticipantsNotReached.
    ...
    READ ENTITY IN LOCAL MODE ZR_Registration_5
         FIELDS ( EventUuid )
         WITH CORRESPONDING #( keys )
         RESULT DATA(registrations).

    LOOP AT registrations INTO DATA(registration).
      DATA all_events     TYPE TABLE OF zevent_05.
      DATA filtered_event TYPE zevent_05.
      ...
      DATA all_registrations     TYPE TABLE OF zregistration_05.
      DATA filtered_registrations TYPE TABLE OF zregistration_05.

      SELECT *
        FROM zregistration_05
      ...
      IF filtered_event-max_participants <= lines( filtered_registrations ).
        message = NEW zcm_event_5( textid = zcm_event_5=>max_participants_reached ).
...
CLASS lhc_Event IMPLEMENTATION.
  ...
  METHOD GenerateEventId.
    SELECT FROM zevent_05 FIELDS MAX( event_id ) INTO @DATA(max_event_id).
    DATA(event_id) = max_event_id + 1.

    MODIFY ENTITY IN LOCAL MODE zr_event_5
  ...
  METHOD CloseEvent.
    READ ENTITY IN LOCAL MODE zr_event_5
         ALL FIELDS
         WITH CORRESPONDING #( keys )
         RESULT DATA(events).
  ...
  METHOD ValidateDates.
    ...
    LOOP AT events INTO DATA(event).
      IF event-EndDate < event-StartDate.
        message = NEW zcm_event_5( textid = zcm_event_5=>invalid_dates ).
  ...
  METHOD ValidateStartDate.
    ...
    LOOP AT events INTO DATA(event).
      IF event-StartDate < lv_date.
        message = NEW zcm_event_5( textid = zcm_event_5=>invalid_startdate ).
        APPEND VALUE #( %tky = event-%tky
                        %msg = message ) TO reported-event.
        APPEND VALUE #( %tky = event-%tky ) TO failed-event.
      ENDIF.
